...
int main(int argc, char *argv[])
{
  ...
  for (unsigned i = 0; i < height; ++i)
  {
    for (unsigned j = 0; j < width; ++j)
    {
      ...
      rgb_image[0 + 3 * (j + width * (height - 1 - i))] = 255.0 * clamp(rgb(0));
      rgb_image[1 + 3 * (j + width * (height - 1 - i))] = 255.0 * clamp(rgb(1));
      rgb_image[2 + 3 * (j + width * (height - 1 - i))] = 255.0 * clamp(rgb(2));
    }
  }
